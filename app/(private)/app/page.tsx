...
export default async function Page() {
  ...
  return (
    <div className="md:grid md:grid-cols-3 flex flex-col gap-4">
      <Link
        href="/app/new"
        className="w-full h-full border border-dashed flex justify-center items-center"
      >
        <div className="flex flex-col justify-center items-center text-center gap-3 text-zinc-600 font-bold min-h-[200px]">
          <TbPlus />
      ...
      {projects.map(project => (
        ...
        <Image fill src={project.cover} alt="Cover" className="rounded object-cover"  />
          </div>
          <div className="p-5 rounded-b space-y-1">
        <p className="text-2xl font-bold text-zinc-800">{project.name}</p>
